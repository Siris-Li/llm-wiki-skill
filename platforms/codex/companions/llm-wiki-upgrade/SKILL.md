...
metadata:
  ...
  source_path: platforms/codex/companions/llm-wiki-upgrade
  upstream: https://github.com/sdyckjq-lab/llm-wiki-skill.git
  target_skill: llm-wiki
allowed-tools:
  - Bash
  - Read
---
...
```bash
SKILL_DIR="${LLM_WIKI_SKILL_DIR:-$HOME/.agents/skills/llm-wiki}"
...
如果 `SKILL_DIR` 不存在，告知用户尚未在 `.agents/skills/llm-wiki` 安装 llm-wiki；如需更新其他位置，先设置 `LLM_WIKI_SKILL_DIR`，然后停止流程。

### Step 2：Clone 最新版本到临时目录

```bash
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/Siris-Li/llm-wiki-skill.git "$TMP_DIR/llm-wiki-skill" 2>&1
echo "CLONE_EXIT=$?"
```
...
```bash
bash "$TMP_DIR/llm-wiki-skill/install.sh" --upgrade --platform codex --target-dir "$SKILL_DIR" 2>&1
echo "UPGRADE_EXIT=$?"
```
...
```bash
bash "$SKILL_DIR/install.sh" --upgrade --platform codex --target-dir "$SKILL_DIR" --with-optional-adapters
```
