...
- 无现行宿主消费者的 learning、收藏、笔记队列、旧存储和 Atlas 旧视角出口及专属测试。
...
|---|---:|---:|---:|
| `dist/engine.esm.js` | 438043 bytes | 421646 bytes | -16397 bytes |
| `dist/engine.iife.js` | 513949 bytes | 500990 bytes | -12959 bytes |
...
- 离线多选和社区选区通过现行 Sigma 节点命中目标验证，不再等待退休 DOM/SVG 根。

`tests/issue-282-performance-acceptance.sh` 顺序复跑通过：

| 路线 | 数据形状 | #272 基线中位数 | #283 中位数 | 上限 | 结果 |
|---|---|---:|---:|---:|---|
| 生产 Sigma | 1k sparse | 89.5 ms | 80.7 ms | 139.5 ms | 通过 |
| 隔离 Sigma | 1k sparse | 52.3 ms | 69.3 ms | 102.3 ms | 通过 |
| 隔离 Sigma | 5k sparse | 152.6 ms | 154.8 ms | 202.6 ms | 通过 |
| 隔离 Sigma | 10k aggregation | 257.9 ms | 245.8 ms | 309.5 ms | 通过 |
...
- 代码审查发现并修复：产物扫描缺少被删模块路径和一个退休出口、离线既有主题记录缺少启动前预置读取、退休源路径与 learning 出口缺少长期不存在性门禁、语义可见性仍携带无用布局参数，以及一个测试 seam 命名不清。
