...
|---|---:|---:|---:|
| `dist/engine.esm.js` | 438043 bytes | 421658 bytes | -16385 bytes |
| `dist/engine.iife.js` | 513949 bytes | 500998 bytes | -12951 bytes |
...
|---|---|---:|---:|---:|---|
| 生产 Sigma | 1k sparse | 89.5 ms | 91.2 ms | 139.5 ms | 通过 |
| 隔离 Sigma | 1k sparse | 52.3 ms | 55.1 ms | 102.3 ms | 通过 |
| 隔离 Sigma | 5k sparse | 152.6 ms | 156.1 ms | 202.6 ms | 通过 |
| 隔离 Sigma | 10k aggregation | 257.9 ms | 293.4 ms | 309.5 ms | 通过 |
...
- 代码审查发现并修复：产物扫描缺少被删模块路径、离线既有主题记录缺少启动前预置读取、退休源路径缺少长期不存在性门禁，以及一个测试 seam 命名不清。
