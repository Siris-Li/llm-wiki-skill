...
**范围**：
- G1-1 关系类型上边（颜色 = 关系类型、矛盾避 ENTITY 红；虚实 = 置信度，数据支持才做；全局低权重、聚焦才完整呈现）
...
**实施**：L 级 phased plan（P0 核验+基线 → P1 工具条 → P2 聚焦 → P3 关系边 → P4 验收），分支 `feat/graph-evolution-1`，Codex `/goal` 执行（plan/progress 本地不入库，沿用 planning-docs local-only 惯例）。
