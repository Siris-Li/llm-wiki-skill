...
### 阶段 4.6：图谱演进第一批（看清关系 + 层层聚焦 + 控制工具条）🚧 设计完成 2026-06-14，待实施
...
**实施**：L 级 phased plan（P0 基线+完整边契约+web test 入口 → P1 工具条 → P2 聚焦 → P3 关系边 → P4 验收），分支 `feat/graph-evolution-1`，Codex `/goal` 执行（plan/progress 本地不入库，沿用 planning-docs local-only 惯例）。

**与既有决策的关系**：修订 ADR-21 / D4.5-6（社区交互：选中高亮 → 进入聚焦视图；左上浮层 → 顶部工具条）；关系边可视化为新决策（实现后补 ADR）。
...
#### 图谱演进候选池（2026-06-13 全景分析沉淀；首批已立项为「阶段 4.6」，见上文 §阶段 4.6）
...
| 关系类型上边 | 按关系词汇表（实现/依赖/对比/矛盾）给边颜色，按置信度给边虚实，"矛盾"边避开 ENTITY 红 | 关系词汇表与置信度体系已有；首批 4.6 先补齐边数据契约再上图 |
...
### 阶段 4.6：图谱演进第一批 🚧 设计完成 2026-06-14，待实施

**当前状态**：设计与执行计划已审查加固，分支 `feat/graph-evolution-1` 执行。首要前置是补齐边数据契约：关系类型和置信度分字段，后续渲染用关系类型控制颜色、置信度控制虚实。
