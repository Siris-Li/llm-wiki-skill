...
| "摘要"区块 | 去 | 摘要 = 正文首段的复制品，正文就在下面，派生信息不重复 |
...
- hover/选中态：展开为现有完整卡片
- 密度策略保持不动（大库自动降级到紧凑标签/圆点的逻辑已在），本条只是把"卡片档"瘦一号
...
| 节点 hover 摘要 tooltip | 留待后续（摘要的正确住所，但不阻塞本阶段） |
...
6. **节点**：默认一行 + 色条；hover 展开完整卡；dense fixture 下密度降级正常
