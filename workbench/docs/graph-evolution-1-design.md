...
边按关系词汇表把"是什么关系""有多确定"画出来，**双维度编码**：

- **颜色 = 关系类型**，但**克制编码**（避免与节点色抢视觉——节点已用左色条编码 ENTITY/SOURCE 等类型）：
  - 对立关系给警示色：**矛盾 = 红**、对比 = 琥珀
  - 顺承关系（实现 / 依赖 / 衍生）= 统一中性色（蓝灰，跟随主题）
  - hover 边时浮出关系词中文，让中性色边也能查到具体类型
- **虚实 = 置信度**：`EXTRACTED` 实线 / `INFERRED` 虚线（"猜的"一眼可辨）
- **不做方向箭头**（有向信息 `from/to` 已在数据，箭头增加视觉噪音，需要时再加）
...
| 操作 | 行为 |
|---|---|
| 单击空白 | 退出节点高亮，**回到当前社区聚焦视图**（不回全图——这是用户明确要的"误操作不打回原形"） |
| 双击空白 | 回全图（沿用 4.5 D4.5-1 已有手势） |
| Esc | 逐级退出：节点高亮 → 社区视图 → 全图 |
...
├── render/   关系边上色/虚实渲染 + hover 关系词；边图例；控制工具条 + 弹出面板（半透明/默认收起）；
│             离线 HTML 工具条置顶
...
1. **关系边**：边按类型着色——矛盾红、对比琥珀、顺承关系中性色；`INFERRED` 边显虚线、`EXTRACTED` 实线；hover 边浮出关系词中文；山水 / 墨夜两主题下均与节点色不冲突
...
| 风险 | 对策 |
|---|---|
| 5 类关系色 + 节点色 → 图面过花 | G1-1 已克制：仅对立关系（矛盾/对比）独立色，顺承关系统一中性；实现时双主题下调试取证 |
