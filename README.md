...
| | 功能 | 说明 |
|---|---|---|
| 🧮 | **共享绘制策略** | 密度、节点显示、标签与关系预算、稳定骨架和社区层级由一份规则产出；Sigma 全局图谱和社区阅读直接使用这份结果，每次更新只计算一次 |
...
- **图谱共享绘制策略** — 密度、节点显示、标签与关系预算、稳定骨架、社区层级和标签方向由同一份规则产出；模型、布局、可见结果和绘制规则在一次更新中各计算一次，Sigma 全局图谱与社区阅读直接使用准备结果，工作台和离线图谱保持一致
