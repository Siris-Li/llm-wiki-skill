...
[![version](https://img.shields.io/badge/v3.6.46-浏览器地基-E8D5B5?style=flat-square&labelColor=3a3026&color=E8D5B5)](https://github.com/sdyckjq-lab/llm-wiki-skill/releases)
...
| ✅ | **工作台可靠启动** | 后台只监听本机，失败启动不会替换现有凭证，重启会换新并恢复上次知识库；退出时会结束持续连接和图谱重建进程 |
| 🧪 | **工作台真实浏览器检查** | 自动启动真实前台和后台，通过真实请求与事件流验证虚构知识库和对话；不使用真实密钥，不把测试替身带进正式构建 |
| 🔎 | **工作台可靠检查** | 前台构建与类型检查每次都核对当前代码，不会因旧结果漏掉图谱或工具状态问题 |
...
- **工作台可靠启动** — 正式启动和自动检查走同一套恢复与关闭流程；失败启动不改现有凭证，退出会结束持续连接和图谱重建进程，检查主动阻止真实用户资料读取、临时目录外写入和外网访问
- **工作台真实浏览器检查** — 使用一次性用户环境、真实前后台、真实端口、HTTP 和事件流验证虚构知识库与对话；外部模型和系统文件夹选择器由测试边界替代，普通启动和正式构建不包含这些替身
- **工作台可靠检查** — 前台构建与类型检查不会复用过期结果，图谱读取和工具摘要的契约不一致会在提交前稳定暴露
