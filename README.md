...
| | 功能 | 说明 |
|---|---|---|
| 🧮 | **共享绘制策略** | 密度、节点显示、标签与关系预算、稳定骨架和社区层级由一份规则产出；工作台、离线图谱与两条显示路线保持相同结果，每次更新只计算一次 |
...
- **图谱共享绘制策略** — 密度、节点显示、标签与关系预算、稳定骨架、社区层级和标签方向由同一份规则产出；模型、布局、可见结果和绘制规则在一次更新中各计算一次，Sigma、DOM/SVG、工作台和离线图谱保持一致
